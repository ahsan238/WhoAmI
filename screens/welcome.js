...
export default class Welcome extends Component {
    ...
    displayResults = (response) => {
        const data_old = response.outputs[0].data
        const data = (data_old.regions[0].data.concepts)
        // console.log(data)
        if (isEmpty(data)) {
            alert("No face found")
            this.setState({welcome: true})
            ...
            // // console.log(ref)
            const boundingBox = data_old.regions[0].region_info.bounding_box;
            ...
            const ageIndex = (array.findIndex(data,function(o) {return o.vocab_id=="age_appearance"}));
            const age = data[ageIndex].name

            const genderIndex = (array.findIndex(data,function(o) {return o.vocab_id=="gender_appearance"}))
            const gender = data[genderIndex].name

            const bloodlineIndex = (array.findIndex(data,function(o) {return o.vocab_id=="multicultural_appearance"}))
            const bloodLine = data[bloodlineIndex].name
